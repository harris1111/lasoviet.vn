...
type AnonymousActorId = Brand<string, "AnonymousActorId">;
...
type CurrentActor =
  | { kind: "account"; userId: UserId; sessionId: SessionId; requestId: string }
  | {
      kind: "anonymous";
...
| P00-T03 | `resolveLocale(pathname: string, cookieLocale?: string): Locale`; VI/EN JSON trees; Next.js 16 proxy request | Canonical VI root and `/en` English routes through `localePrefix: "as-needed"`; errors `UNSUPPORTED_LOCALE`, `I18N_KEY_MISMATCH`, `I18N_TOKEN_MISMATCH` | Locale cookie only; no localized domain values | Root Vietnamese route avoids `/vi`; English route uses `/en`; missing EN key and incompatible `{name}` interpolation both fail parity |
...
| P00-T05 | Approved Blueprint v1.1, `config/route-registry.yml`, product catalog, ordered analytics dictionary, and brand tokens | Typed YAML route loader, `RouteDefinitionV1`, `PublicContentV1`, canonical `AnalyticsEventV1`, canonical funnel, route-state enum, CSS tokens; errors `ROUTE_REGISTRY_INVALID`, `ROUTE_INTENT_COLLISION`, `ROUTE_STATE_INVALID`, `CONTENT_METADATA_INVALID`, `ANALYTICS_EVENT_INVALID`, `ANALYTICS_FUNNEL_INVALID` | Version-controlled registries and content metadata only | Duplicate canonical intent, TypeScript/YAML route duplication, private route in sitemap, reserved SKU marked purchasable, reordered canonical funnel, unknown event property, or indexable route without reviewed content all fail |

## Phase 01 Contracts

| ID | Exact inputs | Outputs and errors | Persistence/events | Required failing vector |
|---|---|---|---|---|
| P01-T01 | `runMigrations(databaseUrl: string)`; `enqueueOutbox(tx, WorkflowEnvelopeV1)` | `Database`; migration result with applied versions; errors `MIGRATION_FAILED`, `OUTBOX_DUPLICATE_KEY` | Auth, anonymous ownership/expiry, privacy, profile, outbox, audit tables; outbox unique on `eventId` and domain idempotency key | Empty DB and previously migrated DB converge to the same schema; anonymous profile ownership cannot exist without expiry or linked account |
| P01-T02 | `createInternalActorToken(CurrentActor, requestId)`; API bearer token; anonymous/account-link request; verification/reset email command | Server-resolved `CurrentActor`; `linkAnonymousActorToAccount()`; `EmailProvider.send(message, idempotencyKey)`; errors `EMAIL_UNVERIFIED`, `SESSION_REVOKED`, `ANONYMOUS_LINK_CONFLICT`, `ACTOR_TOKEN_EXPIRED`, `ACTOR_TOKEN_AUDIENCE`, `ACTOR_TOKEN_INVALID`, `SMTP_RETRYABLE`, `SMTP_CONFIG_INVALID` | Better Auth anonymous/user/account/session rows and notification idempotency state; no token body persistence | Linking preserves one profile/chart owner without duplication; tampered actor token is rejected; browser input cannot mark email verified or expose reset token |
| P01-T03 | `recordConsent(actor, documentVersion, purpose)`; account deletion timestamps; anonymous expiry/manual-delete request | Consent record; account deletion state; anonymous purge result; errors `CONSENT_VERSION_UNKNOWN`, `DELETION_ALREADY_REQUESTED`, `DELETION_RECOVERY_EXPIRED`, `ANONYMOUS_NOT_EXPIRED`, `ANONYMOUS_ALREADY_LINKED` | Versioned consent, deletion request, audit row; purge outbox after account or anonymous deadline | Unlinked anonymous data exists before 24h, purges at 24h, deletes immediately on request, and is retained under account policy only after successful linking |
| P01-T04 | `normalizeBirthProfile(input: BirthProfileV1)`; `resolveZiweiTimeIndex(profile)`; server-resolved account or anonymous actor | `NormalizedBirthProfileV1`; `Result<number, "TIME_UNKNOWN" | "TIME_RANGE_MULTIPLE_BRANCHES" | "INVALID_TIMEZONE" | "INVALID_CALENDAR_INPUT">` | Original input plus immutable normalized revision, actor ownership, expiry when anonymous, and provenance | Range inside one branch succeeds with limitation; crossing range returns `TIME_RANGE_MULTIPLE_BRANCHES`; browser-supplied owner ID is ignored |
...
| ID | Exact inputs | Outputs and errors | Persistence/events | Required failing vector |
|---|---|---|---|---|
| P03-T01 | `privateApiClient(actor, requestId)`; typed loader for `config/route-registry.yml` | Versioned server-only API client, route metadata, robots policy, sitemap index and children; errors `PRIVATE_API_UNREACHABLE`, `ROUTE_REGISTRY_INVALID`, `SITEMAP_SECTION_INVALID` | No browser API host; crawl artifacts generated from YAML registry | Built client bundle contains no private API hostname; private/reserved routes are absent from robots-linked sitemap children; every public route has VI/EN metadata |
| P03-T02 | Account or anonymous `CurrentActor`, `BirthProfileV1`, consent version | `saveBirthProfile(): Result<{ profileId, revisionId, ziweiEligibility, expiresAt? }, "CONSENT_REQUIRED" | "VALIDATION_FAILED" | "PROFILE_FORBIDDEN" | "ANONYMOUS_EXPIRED">` | Profile revision, consent, anonymous expiry when applicable, privacy-safe analytics event | Guest completes chart without registration; linking preserves one owner; manual delete and 24-hour expiry remove access; unknown time cannot reach checkout |
| P03-T03 | Account or unexpired anonymous actor, `ChartId`, selected `EvidenceItemV1` | Owner-authorized chart view; errors `CHART_NOT_FOUND`, `CHART_FORBIDDEN`, `EVIDENCE_NOT_FOUND`, `ANONYMOUS_EXPIRED` | No new business row; access request log excludes chart content | Another actor and an expired anonymous actor receive not-found-equivalent responses with no sequential-ID disclosure |
| P03-T04 | Actor-authorized chart/evidence version and server product catalog | `buildFreeIdentityPreview()` plus topic-selection view containing only `ZIWEI-IDENTITY-P0`; errors `INSUFFICIENT_EVIDENCE`, `CHART_FORBIDDEN`, `SKU_UNAVAILABLE` | Ordered privacy-safe funnel events only | Preview contains 3 linked insights and 10-15% real excerpt; selector is noindex; reserved SKU fails; analytics rejects birth/chart/report fields |
| P03-T05 | Canonical route registry, locale, and approved public content record | Server-rendered page, metadata, canonical/alternate links, breadcrumbs, schema, and route-state disposition; errors `PUBLIC_CONTENT_NOT_FOUND`, `ROUTE_RESERVED`, `ROUTE_ARCHIVED`, `SCHEMA_CONTENT_MISMATCH` | No business persistence; route access metrics use approved event properties | Reserved commercial route returns not-found and is absent from navigation/sitemap; VI root and `/en` alternate metadata agree |
| P03-T06 | Gate 1 route list, VI/EN content files, source registry, reviewer registry | Sixteen core public/trust content records, including both knowledge hubs, and ten reviewed foundation articles per locale; errors `CONTENT_INCOMPLETE`, `CONTENT_SOURCE_MISSING`, `CONTENT_REVIEW_MISSING`, `CONTENT_THIN`, `CONTENT_FORBIDDEN_CLAIM` | Version-controlled content and review metadata | Placeholder, missing knowledge hub, fake expert/testimonial, fear CTA, missing translation, orphan link, or duplicate intent fails the content gate |

## Phase 04 Contracts

| ID | Exact inputs | Outputs and errors | Persistence/events | Required failing vector |
|---|---|---|---|---|
| P04-T01 | Verified dated SePay provider contract; complete server-only SePay environment group; `createOrder(actor, chartId, sku)`; server catalog | `PaymentProvider.createPayment(order)` and checkout result; errors `SEPAY_PREFLIGHT_INCOMPLETE`, `SEPAY_CONFIG_INVALID`, `SKU_UNAVAILABLE`, `CHART_FORBIDDEN`, `ZIWEI_TIME_INELIGIBLE`, `ENTITLEMENT_EXISTS` | Provider contract record contains no secrets; pending order uses server price/provider reference; checkout route becomes `live_noindex` | Luna receives no adapter instruction when provider semantics are unverified; checkout route cannot be indexable or appear in any sitemap |
| P04-T02 | Raw SePay body, exact headers from the verified contract, receipt timestamp | `handleSePayWebhook(): Result<{ paymentId, orderId, duplicate }, "SIGNATURE_INVALID" | "ORDER_NOT_FOUND" | "AMOUNT_MISMATCH" | "EVENT_CONFLICT">` | Payment event, paid order, entitlement, report reservation, and `report.generation.requested.v1` in one transaction | Two concurrent valid deliveries produce one payment, entitlement, report version, and outbox event |
...
| P06-T06 | Production origin, typed `config/route-registry.yml` loader, Gate 1 content evidence, founder-controlled indexing access | Signed public route inventory and indexing handoff; errors `PUBLIC_ROUTE_HTTP_INVALID`, `CANONICAL_MISMATCH`, `SITEMAP_MEMBERSHIP_INVALID`, `PRIVATE_ROUTE_INDEXABLE`, `PUBLIC_CONTENT_NOT_READY` | Release evidence only; no automated external-account mutation | Missing knowledge hub, reserved route in sitemap, private route without noindex/access control, duplicate canonical, missing alternate, or unreviewed content blocks indexing |
